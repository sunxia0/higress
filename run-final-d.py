...
records = {"source_sha": head, "source_tree_clean": True, "started_at": now(), "commands": [], "toolchain": {}}
...
for name, cmd, overrides in commands:
    ...
    with (evidence / item["log"]).open("w") as log:
        run = subprocess.run(cmd, cwd=repo, env=dict(os.environ, **overrides), stdout=log, stderr=subprocess.STDOUT)
...
for name in ["plugin.wasm", "unit.log", "race.log", "extension.log", "interop.log", "wasm.log", "release-unit.log", "catalog.log"]:
    file = evidence / name
    if file.exists():
