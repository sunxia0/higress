...
record={'source_sha':head,'source_tree_clean':True,'token_overrides_unset':True,'command':cmd,'environment_overrides':{'RUNTIME_EVIDENCE':str(out/'runtime')},'started_at':now()}
with (out/'command.log').open('w') as log:
 result=subprocess.run(cmd,cwd=repo,env=dict({k:v for k,v in os.environ.items() if k not in ('GH_TOKEN','GITHUB_TOKEN')},**record['environment_overrides']),stdout=log,stderr=subprocess.STDOUT)
