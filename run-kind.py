...
root=repo.parent/"higress-mcp-auto-tuaqf6zu"
...
record={"source_sha":head,"started_at":now(),"commands":[],"adaptation":"../kind/environment-adaptation.json"}
env=dict(os.environ,KUBECONFIG=str(root/"kind/kubeconfig"),KIND_EXPERIMENTAL_PROVIDER="podman")
...
try:
 ...
 run("start-node",["podman","start","mcp-auto-4684-control-plane"])
 run("load-python",["kind","load","image-archive","--name","mcp-auto-4684",str(root/"kind/python.tar")])
 ...
 run("copy-test",["podman","cp",str(out/"conformance.test"),"mcp-auto-4684-control-plane:/conformance.test"])
 code=run("conformance",["podman","exec","-e","KUBECONFIG=/etc/kubernetes/admin.conf","mcp-auto-4684-control-plane","/conformance.test","-test.v","-test.timeout=8m","-isWasmPluginTest=true","-wasmPluginType=GO","-wasmPluginName=mcp-server","--ingress-class=higress","--debug=true","--test-area=run","--execute-tests=WasmPluginsMCP20260728"],check=False)
 ...
 run("image-identities",["podman","exec","mcp-auto-4684-control-plane","ctr","-n","k8s.io","images","list"],check=False)
